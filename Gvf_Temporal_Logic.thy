...
type_synonym \<Phi>\<^sub>T\<^sub>M = \<open>Atom\<^sub>M \<Phi>\<^sub>T\<close>

abbreviation \<open>B\<^sub>T \<Phi> \<equiv> Atom (Bl \<Phi>)\<close>
abbreviation \<open>G\<^sub>T \<Phi> \<equiv> Atom (Gl \<Phi>)\<close>
...
fun to_\<Phi>\<^sub>T :: \<open>'a \<Phi>\<^sub>P \<Rightarrow> 'a \<Phi>\<^sub>T\<close> (\<open>_\<^sup>T\<close> [100]) where
  ...
  \<open>\<^bold>\<bottom>\<^sup>T = \<bottom>\<^sub>T\<close> |
...
fun semantics\<^sub>T :: \<open>trace \<Rightarrow> nat \<Rightarrow> \<Phi>\<^sub>T\<^sub>M \<Rightarrow> bool\<close> (\<open>_, _ \<Turnstile>\<^sub>T\<close> [50]) where
  \<open>s, i \<Turnstile>\<^sub>T \<bottom>\<^sub>T = False\<close> |
  \<open>s, i \<Turnstile>\<^sub>T (Atom x) = ((\<Phi>\<^sub>P.Atom x)\<^bold>[s i\<^bold>]\<^sub>M)\<close> |
...
lemma sem\<^sub>T\<^sub>M_equiv: \<open>(\<forall>M. semantics\<^sub>M M p) \<longleftrightarrow> (\<forall>s i. (p\<^bold>[s i\<^bold>]\<^sub>M))\<close> 
  using st_nth.simps(1) by metis

lemma sem\<^sub>T\<^sub>M_equiv2: \<open>s, i \<Turnstile>\<^sub>T (p\<^sup>T) \<longleftrightarrow> (p\<^bold>[s i\<^bold>]\<^sub>M)\<close> 
  by (induct p) auto

lemma \<open>(\<forall>M. semantics\<^sub>M M p) \<longleftrightarrow> (\<forall>s i. s, i \<Turnstile>\<^sub>T (p\<^sup>T))\<close> using sem\<^sub>T\<^sub>M_equiv sem\<^sub>T\<^sub>M_equiv2 by simp

abbreviation semantics_trace\<^sub>T\<^sub>M :: \<open>trace set \<Rightarrow> \<Phi>\<^sub>T\<^sub>M \<Rightarrow> bool\<close> (infix \<open>\<Turnstile>\<^sub>T#\<close> 50) where
...
theorem unless_ht: \<open>(\<forall>b \<in> \<Pi>. (\<Turnstile>\<^sub>H \<^bold>{ \<phi> \<^bold>\<and> \<^bold>\<not> \<psi> \<^bold>} ((fst b) \<triangleright> do (snd b)) \<^bold>{ \<phi> \<^bold>\<or> \<psi> \<^bold>})) \<longleftrightarrow> Agent \<Turnstile>\<^sub>T# \<phi>\<^sup>T unless \<psi>\<^sup>T\<close>
proof
  ...
  show \<open>Agent \<Turnstile>\<^sub>T# \<phi>\<^sup>T unless \<psi>\<^sup>T\<close> (is \<open>Agent \<Turnstile>\<^sub>T# ?\<phi> unless ?\<psi>\<close>)
  proof (rule ccontr)
    ...
    proof
      ...
      then have \<open>st_nth s (k - 1) \<Turnstile>\<^sub>M \<phi> \<^bold>\<and> \<^bold>\<not> \<psi>\<close> by (simp add: sem\<^sub>T\<^sub>M_equiv2 transfer_semantics\<^sub>M)
      ...
      then have \<open>st_nth s k \<Turnstile>\<^sub>M \<^bold>\<not> \<phi> \<^bold>\<and> \<^bold>\<not> \<psi>\<close> by (simp add: sem\<^sub>T\<^sub>M_equiv2 transfer_semantics\<^sub>M)
    ...
    next
      ...
      then have \<open>st_nth s (j - 1) \<Turnstile>\<^sub>M \<phi> \<^bold>\<and> \<^bold>\<not> \<psi>\<close> by (simp add: sem\<^sub>T\<^sub>M_equiv2 transfer_semantics\<^sub>M)
      moreover 
      {
        ...
        then have \<open>st_nth s j \<Turnstile>\<^sub>M \<^bold>\<not> \<phi> \<^bold>\<and> \<^bold>\<not> \<psi>\<close> by (simp add: sem\<^sub>T\<^sub>M_equiv2 transfer_semantics\<^sub>M)
      }
  ...
  qed
next
  assume \<open>Agent \<Turnstile>\<^sub>T# \<phi>\<^sup>T unless \<psi>\<^sup>T\<close> (is \<open>Agent \<Turnstile>\<^sub>T# ?\<phi> unless ?\<psi>\<close>)
      ...
      proof
        ...
        proof
          ...
          proof (cases \<open>st_nth s i \<Turnstile>\<^sub>M \<phi>\<close>)
            case True
            then have \<open>s, i \<Turnstile>\<^sub>T ?\<phi>\<close> by (simp add: sem\<^sub>T\<^sub>M_equiv2 transfer_semantics\<^sub>M)
            ...
            proof (cases \<open>j > i + 1\<close>)
              ...
              then have \<open>st_nth s (i + 1) \<Turnstile>\<^sub>M \<phi> \<^bold>\<or> \<psi>\<close> by (simp add: sem\<^sub>T\<^sub>M_equiv2 transfer_semantics\<^sub>M)
            ...
            next
              ...
              proof (cases \<open>st_nth s i \<Turnstile>\<^sub>M \<psi>\<close>)
                case False
                then have \<open>\<not> s, i \<Turnstile>\<^sub>T ?\<psi>\<close> by (simp add: sem\<^sub>T\<^sub>M_equiv2 transfer_semantics\<^sub>M)
                with ** have \<open>s, i + 1 \<Turnstile>\<^sub>T ?\<psi> \<or> s, i + 1 \<Turnstile>\<^sub>T ?\<phi>\<close> by simp
                then show ?thesis using sem\<^sub>T\<^sub>M_equiv2 transfer_semantics\<^sub>M by auto
              qed simp
            qed
          qed simp
        qed
...
theorem ex_ensures:
  assumes \<open>\<forall>b \<in> \<Pi>. (\<Turnstile>\<^sub>H \<^bold>{ \<phi> \<^bold>\<and> \<^bold>\<not> \<psi> \<^bold>} ((fst b) \<triangleright> do (snd b)) \<^bold>{ \<phi> \<^bold>\<or> \<psi> \<^bold>})\<close> 
    ...
    shows \<open>Agent \<Turnstile>\<^sub>T# \<phi>\<^sup>T ensures \<psi>\<^sup>T\<close> (is \<open>Agent \<Turnstile>\<^sub>T# ?\<phi> ensures ?\<psi>\<close>)
proof
  fix s
  assume \<open>s \<in> Agent\<close>
...
inductive leads_to :: \<open>\<Phi>\<^sub>T\<^sub>M \<Rightarrow> \<Phi>\<^sub>T\<^sub>M \<Rightarrow> bool\<close> (infix \<open>\<mapsto>\<close> 55) where
  ensures: \<open>\<forall>s \<in> Agent. \<forall>i. s, i \<Turnstile>\<^sub>T (\<phi> ensures \<psi>) \<Longrightarrow> \<phi> \<mapsto> \<psi>\<close> |
...
lemma leads_to_temp: \<open>\<phi> \<mapsto> \<psi> \<Longrightarrow> \<forall>s \<in> Agent. \<forall>i. s, i \<Turnstile>\<^sub>T (\<phi> \<longrightarrow>\<^sub>T \<diamondop>\<psi>)\<close>
proof (induct rule: leads_to.inducts)
...
next
  ...
  proof
    fix s
    assume \<open>s \<in> Agent\<close>
    show \<open>\<forall>i. s, i \<Turnstile>\<^sub>T (\<phi> \<longrightarrow>\<^sub>T \<diamondop>\<psi>)\<close>
    proof
      fix i
      have \<open>s, i \<Turnstile>\<^sub>T \<phi> \<longrightarrow> s, i \<Turnstile>\<^sub>T (\<diamondop> \<psi>)\<close> 
      proof
        assume \<open>s, i \<Turnstile>\<^sub>T \<phi>\<close>
        with \<open>s \<in> Agent\<close> have \<open>s, i \<Turnstile>\<^sub>T (\<diamondop> \<chi>)\<close> using imp(2) by simp
        then obtain j where \<open>j \<ge> i\<close> \<open>s, j \<Turnstile>\<^sub>T \<chi>\<close> unfolding eventuality_def always_def by auto
        with \<open>s \<in> Agent\<close> have \<open>s, j \<Turnstile>\<^sub>T (\<diamondop> \<psi>)\<close> using imp(4) by simp
        then obtain k where \<open>k \<ge> j\<close> \<open>s, k \<Turnstile>\<^sub>T \<psi>\<close> unfolding eventuality_def always_def by auto
        with \<open>j \<ge> i\<close> show \<open>s, i \<Turnstile>\<^sub>T (\<diamondop> \<psi>)\<close> 
          using order.trans unfolding eventuality_def always_def by auto
      qed
      then show \<open>s, i \<Turnstile>\<^sub>T (\<phi> \<longrightarrow>\<^sub>T \<diamondop> \<psi>)\<close> by simp
    qed
  qed
next
